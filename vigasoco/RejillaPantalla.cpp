...
bool RejillaPantalla::obtenerAlturaPosicionesAvanceComun(Personaje *pers, int alturaLocal, int &difAltura1, int &difAltura2, int &avanceX, int &avanceY)
{
	...
	for (int j = 0; j < 4; j++){
		...
		for (int i = 0; i < 4; i++){
			...
			if (alturaPos < 0x10){
				// si no hay un personaje en esa posición,
				// obtiene la diferencia de altura entre la posición y el personaje
	...
	if (!pers->enDesnivel){
		...
		// si el personaje ocupa una posición en la rejilla,
		// guarda la diferencia de altura de las 2 posiciones hacia las que quiere avanzar
