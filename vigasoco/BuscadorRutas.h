...
class BuscadorRutas : public Singleton<BuscadorRutas>
{
	// campos
public:
	// indica si se ha generado algún camino en esta iteración del bucle principal
	bool generadoCamino;	
	bool seBuscaRuta;			// indica si se ejecuta el buscador de rutas o no
	// contador de la animación de guillermo al inicio de esta iteración del bucle principal
	int contadorAnimGuillermo;

	int numAlternativas;			// número de alternativas generadas
	int alternativaActual;			// alternativa que se está probando actualmente
	// nivel de recursión de la última ejecución del algoritmo de búsqueda de caminos
	int nivelRecursion;	
	...
	// objeto para realizar operaciones relacionadas con la rejilla de pantalla
